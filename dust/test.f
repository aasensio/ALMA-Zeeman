	  program test
		...
		call kappa_dust(1d10, 1, 0.1d0, res,.TRUE.)
		print *, res

	  end program test


       subroutine kappa_dust(freq,iddust,adust,kdust,dustok)

c      Input: freq                       [Hz]
c             type of dust 'iddust'
c             dust grains radius 'adust' [um]
c      Returns dust absortion coefficient 'kdust' [cm2/g(dust)]
c              and 'dustok' if called from command interpreter 
       ...
       integer i,j,jl,iddust,ndata(nmaxiddust)
       real*8 freq,adust,x0,n0,k0,wl0,qabs,slope
       ...
       real*8 kdust                      ! dust absortion coefficient [cm2/g(dust)]
       complex*16 ri0

       logical dustok
       logical wlincr                    ! data ordered by increasing(decreasing) wavelentgh

c      iddust=1: AC (amorphous carbon); Suh 2000, MNRAS, 315, 740
c      complex index of refraction (n,k) as a function of wavelength [um]
       ndata(1)=98
...
c      Interpolate/extrapolate complex index of refraction n,k in log units of wavelength 
       ...
       if(wlincr)then ! data ordered by increasing wavelength
          do j=1,ndata(iddust)-1
             if(log10(wl0).ge.log10(wl(iddust,j)).and.
     #       log10(wl0).le.log10(wl(iddust,j+1)))then
                jl=j             
             endif
          enddo
          if(log10(wl0).lt.log10(wl(iddust,1)))jl=1
          if(log10(wl0).gt.log10(wl(iddust,ndata(iddust))))
     #    jl=ndata(iddust)-1
       else           ! data ordered by decreasing wavelength
          do j=1,ndata(iddust)-1
             if(log10(wl0).le.log10(wl(iddust,j)).and.
     #       log10(wl0).ge.log10(wl(iddust,j+1)))then
                jl=j             
             endif
          enddo
          if(log10(wl0).gt.log10(wl(iddust,1)))jl=1
          if(log10(wl0).lt.log10(wl(iddust,ndata(iddust))))
     #    jl=ndata(iddust)-1
       endif
       slope=(n(iddust,jl+1)-n(iddust,jl))/
     # (log10(wl(iddust,jl+1))-log10(wl(iddust,jl)))
       n0=n(iddust,jl)+slope*(log10(wl0)-log10(wl(iddust,jl)))
       slope=(k(iddust,jl+1)-k(iddust,jl))/
     # (log10(wl(iddust,jl+1))-log10(wl(iddust,jl)))
...
c      Compute kdust: k_nu = (3/4)Qabs/(a_dust*rho_dust)
...
c*****************************************************************
c
c      \_______________________   modified for LVG  _________________________/ 
c
       subroutine mie(x,ri,qabs)
c
c      \______________________________  end  ________________________________/ 
      parameter(nterms=300)
      IMPLICIT REAL*8(A-H,O-Z)                                          
      COMPLEX*16 ri
      DIMENSION xx(50)
c   20 FORMAT(2D14.10)                                                 
c   21 FORMAT(3X,'SPHERES: homogeneous')                                 
c   22 FORMAT(3X,'THEORY:  exact')                                        
c  223 FORMAT(I4)                                          
c   24 FORMAT(4X,'x',7X,'Qext',4X,'Qsca',4X,'Qabs',4X,'Qbk',5x,'Qpr',
c     *       4X,'Albedo',3x,'g')
c   25 FORMAT(13X,F8.4,9X,I3,6X,2D15.4,F10.2,5X,D12.3,F11.3)             
c   29 FORMAT(I4/(F8.4))                                                 
c   30 FORMAT(3x,'m = '2F14.10,'*i')
c   31 FORMAT(F8.4)                                                      
c   32 FORMAT(1H  )                                                      
c   33 FORMAT(1X,8(F8.4))
c  233 FORMAT(4X,i4,2F10.3,f11.1)                                             
c   34 FORMAT( 8X,'x = ',F7.3,5x,'Qext = ',f8.4)          
c   36 FORMAT(1X,64 ('='))
c   37 FORMAT(1X,60 ('*')//)                                             
c   38 FORMAT(1X,10I4)                                                   
c   41 FORMAT(1X,64('.'))
c   42 FORMAT(1X,64('-'))
c   44 FORMAT(1X,A8/(1X,2F8.4))                                          
c   48 FORMAT( 5X,F8.4,2X,2(F10.3,3X),3X,F8.2)                           
c
c*                  INPUT
c
c LVG        print *,'start Mie'
c LVG      open (unit=05,file='mie.dat',status='old',access='sequential')
c LVG      open (unit=07,file='mie.out',status='unknown',access='append')
c LVG*      PI = 4d0 * datan(1d0)
c LVG      READ (5,20) ri
c LVG      READ (5,29) nx,(xx(I),I=1,Nx)
c      \_______________________   modified for LVG  _________________________/ 
c
       nx=1
       xx(1)=x
...
c LVG      write (*,42)
  ...
  444   continue
c LVG        write (7,33) x,qext,qsca,qabs,qbk,qpr,alb,g
c LVG        write (*,*) x, qext
  276   continue
c
c LVG 1000 continue
...
c      \_______________________   modified for LVG  _________________________/ 
c
c      NUM2=NM(cDSQRT(RI*DCONJG(RI))*X)
